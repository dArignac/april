...
function april_nav_menu_item_args( $args, $item, $depth ) {
	if ( $item->hasChildren ) {
		...
		$args->after = ' <span data-toggle="collapse" data-target="#tgglr-' . $toggler . ' + ul.sub-menu" id="tgglr-' . $toggler . '" class="toggler pull-xs-right"><span class="sr-only">toggle menu</span></span>';
	} else {
		$args->after = '';
	}
	return $args;
